...
handler <- function(args) {
    ...
    counties_1_state <- counties4project[
        counties4project$STATE == statename,
    ] %>%
        data.frame()
    ...
    if (
        state.name[state.abb == stateAbbrev] %in% unique(countiesUS$STATENAME)
    ) {
    ...
    system.time({
        ...
        temp <- lapply(countynames, function(x) {
            COUNTY <- x
            # print(x)
            countyCD <- counties_1_state$co_fips[
                counties_1_state$COUNTY == COUNTY
            ]
            countynameLC = countiesFIA_1_state$COUNTYNM[
                countiesFIA_1_state$COUNTYCD == countyCD
            ] # should be the same as x
            if (
                any(grepl(" Of ", countynameLC), na.rm = TRUE) |
                    any(grepl(" And ", countynameLC), na.rm = TRUE)
            ) {
    ...
    write.csv(
        chm_dist,
        file.path("CHM_dist_by_county.csv"),
        row.names = FALSE
    )
    ...
    return(psaeRuntimeConnector::new_file("CHM_dist_by_county.csv"))
}
