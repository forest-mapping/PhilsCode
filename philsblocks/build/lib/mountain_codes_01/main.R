...
calc_vol_bio <- function(statecode) {
    ...
    # rbind(df1, df2) %>%
    return(
        df2 %>% dplyr::arrange(STATECD, co_fips, surveyunit, response)
    )
}
...
handler <- function(input_id) {
    ...
    bio_by_fips_su$code <- as.character(
        bio_by_fips_su$`PLOT.STATECD * 1000 + PLOT.COUNTYCD + PLOT.UNITCD *0.1` *
            10
    )
    str(bio_by_fips_su)
    ...
    for (i in seq_along(states)) {
        ...
        df2 <- bio_by_fips_su %>%
            ...
            filter(STATECD == statecode) %>%
            dplyr::arrange(STATECD, co_fips, surveyunit, response)

        if (is.null(fia_estimates)) {
            fia_estimates <- df2
        } else {
            fia_estimates <- rbind(fia_estimates, df2)
        }
    }
