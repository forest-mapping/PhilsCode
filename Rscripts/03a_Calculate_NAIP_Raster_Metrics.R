...
if(length(args)==0){
  ...
  stateAbbrev = ifelse(length(args)==1,args,stop("Program halted. Only one command line argument allowed."))
}
...
counties_1_state <- counties4project[counties4project$STATE == statename,] %>% data.frame()
countiesFIA_1_state <- countiesFIA[countiesFIA$STATECD == stateCD,
...
if(state.name[state.abb == stateAbbrev] %in% unique(countiesUS$STATENAME)){
  ...
  stop(red("command line argument entered: ",args,"has no valid state name in state.name dataset\n"))
}

# states <- data.frame(name=state.name,abbrev=state.abb)

countiesPolygon <- countiesUS[countiesUS$STATENAME == state.name[state.abb == stateAbbrev],]
...
countynames <- unlist(lapply(1:length(mapnames_noWater),function(x)substr(mapnames_noWater[x],start=5,stop=(nchar(mapnames_noWater[x])-12))))

# Loop through counties and tabulate the default CHM distribution by 5 m ht classes  #
system.time({               # runs in about 2 minutes for Virginia counties  #
  temp <- lapply(countynames,function(x){
    COUNTY <- x
    # print(x)
    countyCD <- counties_1_state$co_fips[counties_1_state$COUNTY == COUNTY]
    countynameLC = countiesFIA_1_state$COUNTYNM[countiesFIA_1_state$COUNTYCD == countyCD] # should be the same as x
    ...
    statecd <- statesUS$STATECD[statesUS$STUSAB == args]
    chm.exists = file.exists(file.path(path_chm_noWater,args,paste0("chm_",COUNTY,"_noWater.tif")))
    cat("NAIP noWater CHM for county:",COUNTY,ifelse(chm.exists,"exists","does not exist"),'\n')
    if(chm.exists){
      test_tif <- raster(file.path(path_chm_noWater,args,paste0("chm_",COUNTY,"_noWater.tif")))
      bins <- cut(test_tif[],breaks=c(0,5,10,15,20,25,30,35),labels=F)*5
      test_tif[] <- bins
...
chm_dist <- do.call(rbind,temp) %>% dplyr::left_join(statesUS) %>%
  dplyr::rename(COUNTYNAME = countyname,FREQ = Freq,BIN_HT = bin_ht)

write.csv(chm_dist,file.path(path_chm_noWater,args,"CHM_dist_by_county.csv"),row.names = F)

print(paste0("Default (noWater) CHM bin distributions calculated for: ",statesUS$STATENAME[statesUS$STUSAB == args]))
...
countynames <- unlist(lapply(1:length(mapnames_GEDI),
                             function(x)strsplit(strsplit(mapnames_GEDI[x],c("_"))[[1]][2],".",fixed=TRUE)[[1]][1]))
...
system.time({               # runs in about 2 minutes for Virginia counties  #
  temp <- lapply(countynames,function(x){
    ...
    # print(x)
    countyCD <- counties_1_state$co_fips[counties_1_state$COUNTY == COUNTY]
    if(length(countyCD) > 1)countyCD=countyCD[1]# fix counties like Dickson Tennessee)
    countynameLC = countiesFIA_1_state$COUNTYNM[countiesFIA_1_state$COUNTYCD == countyCD] # should be the same as x
    if(grepl(" Of ",countynameLC) | grepl(" And ",countynameLC)){
      countynameLC <- gsub(" Of "," of ",countynameLC)
      countynameLC <- gsub(" And "," and ",countynameLC)
    ...
    chm.exists = file.exists(file.path(path_chm_GEDI,args,paste0(countyCD,"_",COUNTY,".img")))
    cat("GEDI mask CHM for county:",countyname,ifelse(chm.exists,"exists","does not exist"),'\n')
    if(chm.exists){
      test_tif <- raster(file.path(path_chm_GEDI,args,paste0(countyCD,"_",COUNTY,".img")))
      bins <- cut(test_tif[],breaks=c(0,5,10,15,20,25,30,35),labels=F)*5
      test_tif[] <- bins
      chm_dist <- as.data.frame(table(bins))
      chm_dist$STATECD = statecd
...
write.csv(chm_dist,file.path(path_chm_GEDI,args,"CHM_dist_by_county.csv"),row.names = F)

print(paste0("GEDI CHM bin distributions calculated for: ",statesUS$STATENAME[statesUS$STUSAB == args]))
...
countynames <- unlist(lapply(1:length(mapnames_NLCD),
                             function(x)strsplit(strsplit(mapnames_NLCD[x],c("_"))[[1]][2],".",fixed=TRUE)[[1]][1]))
countynames <- sort(countynames)
...
system.time({               # runs in about 2 minutes for Virginia counties  #
  temp <- lapply(countynames,function(x){
    ...
    statecd <- statesUS$STATECD[statesUS$STUSAB == args]
    countyname = test_poly$NAME # should be the same as x
    chm.exists = file.exists(file.path(path_chm_NLCD,args,paste0(countyCD,"_",COUNTY,".img")))
    cat("NLCD mask CHM for county:",countyname,ifelse(chm.exists,"exists","does not exist"),'\n')
    if(chm.exists){
      test_tif <- raster(file.path(path_chm_NLCD,args,paste0(countyCD,"_",COUNTY,".img")))
      bins <- cut(test_tif[],breaks=c(0,5,10,15,20,25,30,35),labels=F)*5
      test_tif[] <- bins
      chm_dist <- as.data.frame(table(bins))
      chm_dist$STATECD = statecd
...
write.csv(chm_dist,file.path(path_chm_NLCD,args,"CHM_dist_by_county.csv"),row.names = F)

print(paste0("NLCD CHM bin distributions calculated for: ",statesUS$STATENAME[statesUS$STUSAB == args]))
