eblupFH_3 <- function(
  ...
  if (method == "ML") {
    ...
    while ((diff > PRECISION) & (k < MAXITER)) {
      ...
      s <- (-0.5) *
        ...
        0.5 * (t(Py) %*% Py) +
        (1 / m) *
          sum((vardir / (vardir + Aest.ML[k])**2)) /
          (atan(sum(diag(m) - diag(shrinkage)))) *
          (1 + sum((diag(m) - diag((vardir / (vardir + Aest.ML[k])))**2)))
      F <- 0.5 * sum(Vi^2)
    ...
    loglike <- (-0.5) *
      (sum(
        log(2 * pi * (A.ML + vardir)) +
          (resid^2) / (A.ML + vardir)
      )) +
      (1 / m) * log(atan(sum(diag(m) - diag(shrinkage))))
    AIC <- (-2) * loglike + 2 * (p + 1)
    ...
    while ((diff > PRECISION) & (k < MAXITER)) {
      ...
      s <- (-0.5) *
        ...
        0.5 * (t(Py) %*% Py) +
        (1 / m) *
          sum((vardir / (vardir + Aest.REML[k])**2)) /
          (atan(sum(diag(m) - diag(shrinkage)))) *
          (1 + sum((diag(m) - diag((vardir / (vardir + Aest.REML[k])))**2)))
      F <- 0.5 * sum(diag(P %*% P))
  ...
  if (B >= 1) {
    ...
    b <- 1
    while (b <= B) {
      uastb <- sqrt(lambdahat) *
        matrix(data = rnorm(D, mean = 0, sd = 1), nrow = D, ncol = 1)
      eastb <- sqrt(sigma2d) *
        matrix(data = rnorm(D, mean = 0, sd = 1), nrow = D, ncol = 1)
      ...
      if (resultb$fit$convergence == FALSE) {
        ...
        B1ast <- sum(
          (lambdahatsigma2d + Xbetahathatastb2 - yastbXbetahatastb2) /
            lambdahatastbsigma2d
        )
        B1hatast <- B1hatast + B1ast
        logf <- (-0.5) *
          sum(
            log(2 * pi * lambdahatastbsigma2d) +
