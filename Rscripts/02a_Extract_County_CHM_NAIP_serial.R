...
if(!dir.exists(path_naip)) stop("No input path ",path_naip)
if(!dir.exists(path_gedi_default)) stop("No input path ",path_gedi_default)
if(!dir.exists(path_gedi_nlcd)) stop("No input path ",path_gedi_nlcd)
stateAbbrev <- state.abb[state.name==args]
path_naip_default <- file.path(path_naip,"noWater",stateAbbrev)
if(!dir.exists(path_naip_default)) stop("No input path ",path_naip_default)
...
if(!dir.exists(path_chm_GEDI)) dir.create(path_chm_GEDI)
...
counties_1_state <- counties4project[counties4project$STATE == args,] %>% data.frame()
...
naip_gedi_fn <- dir(file.path(path_gedi_default,stateAbbrev),pattern = ".tif$")
...
mask_GEDI_NLCD <- function(i){
  ...
  countyCD <- as.integer(substr(counties_1_state$COVER_ID[counties_1_state$COUNTY == COUNTY],3,5))[1]
  # print(paste(COUNTY %in% counties_1_state$COUNTY,COUNTY,"County"))
  default <- rast(file.path(path_naip_default,naip_default_fn[i]))
  countynameLC <- countiesFIA_1_state$COUNTYNM[countiesFIA_1_state$COUNTYCD == countyCD]
  ...
  gedi_fn <- naip_gedi_fn[grep(x=naip_gedi_fn,pattern=paste0(countynameLC,".tif"))]
  gedi <- rast(file.path(path_gedi_default,stateAbbrev,gedi_fn))
  nlcd_fn <- naip_nlcd_fn[grep(x=naip_nlcd_fn,pattern=paste0(countynameLC,"_forest.tif"))]
  ...
  if(args %in% c("North Carolina","Tennessee","Virginia")){
    raster::writeRaster(naip_gedi,overwrite=TRUE,
                        filename=file.path(path_chm_GEDI,paste0(countyCD,"_",COUNTY,".img")))
    raster::writeRaster(naip_nlcd,overwrite=TRUE,
                        filename=file.path(path_chm_NLCD,paste0(countyCD,"_",COUNTY,".img")))
    
    png(filename=file.path(path_chm_GEDI,paste0(countyCD,"_",COUNTY,".png")),
        width = 4,height=4,units="in",res=300)
    plot(naip_gedi,main=paste0(countynameLC," County, GEDI mask"))
    dev.off()
    png(filename=file.path(path_chm_NLCD,paste0(countyCD,"_",COUNTY,".png")),
        width = 4,height=4,units="in",res=300)
...
stop(paste0("Program complete.\n",args," NAIP CHMs masked and written to ",path_chm_GEDI,"\nand ",path_chm_NLCD))
