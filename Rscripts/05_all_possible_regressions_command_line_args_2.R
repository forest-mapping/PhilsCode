...
all_p <- function(p,preds,resp){
  ...
  system.time({
    test <- lapply(1:n,function(j){
      f = as.formula(paste(resp,"~ ", paste(rhs_all[,j], collapse=" + ")," + 0"))
      print(paste("reg:",j))
      ...
      if((j==80 & statecd == 51 & res == "Biomass" & src == "NAIP_NLCD") |
         (j==889 & statecd == 51 & res == "Volume" & src == "NAIP_NLCD") |
         (j==578 & statecd == 51 & res == "Biomass" & src == "NAIP_NLCD")){
        print(paste0("No convergence in model #",j," of ",p," predictors: ",paste0(rhs_all[,j],collapse = ' ')))
        data.frame(reg=j,preds=rhs_all[,j],aic=NA,pvalue=NA)
      }else{
        if(eblup_fit$fit$convergence == FALSE)eblup_fit <- eblupFH_3(f,vardir=var,data=regdata,MAXITER = 1000)
        if(eblup_fit$fit$convergence == TRUE){
          data.frame(reg=j,preds=rhs_all[,j],aic=eblup_fit$fit$goodness[2],pvalue=eblup_fit$fit$estcoef$pvalue)
        }else{
          print(paste0("No convergence in model #",j," of ",p," predictors: ",paste0(rhs_all[,j],collapse = ' ')))
          data.frame(reg=j,preds=rhs_all[,j],aic=NA,pvalue=NA)
        }
    ...
    for(k in 1:k_best){
      reg_no = all_reg_temp$reg[which(all_reg_temp$aic == min(all_reg_temp$aic,na.rm=TRUE))]
      if(p == 1){
        best_reg[k] = reg_no
      }else{
        best_reg[k]=ifelse(sd(reg_no) < tol,reg_no[1],stop("Problem with best_reg & reg_no in all_p function "))
      }
      # if(k == 1){
...
best_regs <- do.call(rbind,lapply(1:k_best,function(i)all_p(p=i,preds=x,resp=y)))

temp <- best_regs %>% group_by(rank,p) %>% 
  summarize(all_signif = all(is.signif),
            aic = min(aic)) %>%
  filter(all_signif == TRUE) %>%
  arrange(aic,p,rank)

best_preds <- temp[1,] %>% left_join(best_regs) %>% 
  ungroup() %>% dplyr::select(preds) %>%
  unlist()

best_model = c(statecd,src,as.formula(
  paste(res,"~ ", paste(best_preds, collapse=" + ")," + 0")))
if(grepl(src,pattern = "GEDI_")){
  saveRDS(best_model,
          file=file.path("/home/rstudio/data/GEDI",paste0("best_",
                                                          statecd,"_",
                                                          src,"_",
                                                          res,".RDS")))
}else{
  saveRDS(best_model,
          file=file.path("/home/rstudio/data/NAIP",paste0("best_",
                                                          statecd,"_",
                                                          src,"_",
                                                          res,".RDS")))
}
...
if(res == "Volume" & src == "GEDI_NLCD" & statecd == 51){
  ...
  png(file.path("EBLUPS_SE.png"),res=300,width=3.25,height=3.25,units="in")
  par(mfrow=c(1,1),mar=c(2.0,2.1,0.5,0.5)+.1)
  par(mfrow=c(1,1),mar=c(2.5,2.5,0.5,0.5)+.1)
  plot(xx,yy,ann=FALSE,yaxt="n",xaxt="n",cex=.5)
  axis(1,cex.axis=.7,padj=-2,tck=-.02,at=seq(from=0,to=max_x,by=5))
  axis(2,cex.axis=.7,padj=2,tck=-.02,at=seq(from=0,to=max_y,by=5))
  mtext(text=expression(paste("SAE EBLUP Net Volume (million m"^"3",")")),side=2,padj=-2,cex=.6)
  mtext(text=expression(paste("FIA County Net Volume (million m"^"3",")")),side=1,padj=2,cex=.6)
  ...
  par(mfrow=c(1,1),mar=c(2.0,2.1,0.5,0.5)+.1)
  par(mfrow=c(1,1),mar=c(2.5,2.5,0.5,0.5)+.1)
  plot(cv_eblup_FIA~cv_direct_FIA,ann=FALSE,yaxt="n",xaxt="n",cex=.5)
  axis(1,cex.axis=.7,padj=-2,tck=-.02,at=seq(from=0,to=max_x,by=10))
  axis(2,cex.axis=.7,padj=2,tck=-.02,at=seq(from=0,to=max_y,by=2))
  mtext(text=expression(paste("SAE EBLUP Net Volume (million m"^"3",")")),side=2,padj=-2,cex=.6)
  mtext(text=expression(paste("FIA County Net Volume (million m"^"3",")")),side=1,padj=2,cex=.6)
  mtext(text="VA 2019, NLCD 41-43, 90",side=3,padj=2,cex=.8)
  abline(0,1,col="blue")
