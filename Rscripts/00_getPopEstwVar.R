...
getEstimate <- function(tree_table_var= "DRYBIO_AG", base_query, by= "PLOT.STATECD") {
  ...
  # clean up the names
  value$STATECD <- ifelse(nchar(value$eval_grp) == 5,
                          substr(value$eval_grp, 1, 1),
                          substr(value$eval_grp, 1, 2))
  
  value$YEAR <- ifelse(nchar(value$eval_grp) == 5,
...
system.time(
  vol_by_fips_su <- getEstimate("VOLCFGRS", base_tree_query, "PLOT.STATECD * 1000 + PLOT.COUNTYCD+ PLOT.UNITCD *0.1")
)

# total biomass estimate and variances for latest evaluation by county
system.time(
  bio_by_fips_su <- getEstimate("DRYBIO_AG", base_tree_query, "PLOT.STATECD * 1000 + PLOT.COUNTYCD + PLOT.UNITCD *0.1")
)
