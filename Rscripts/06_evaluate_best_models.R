...
refit_best <- function(x) {
  ...
  data.frame(
    ...
    SOURCE = src,
    RESPONSE = res,
    N_PAR = as.integer(nrow(eblup_fit$fit$estcoef)),
    AIC = as.numeric(aic),
    refvar = refvar
  )
...
test <- (data.frame(
  STATECD = unlist(best_models[1:30]),
  SOURCE = unlist(best_models[31:60]),
  RESPONSE = unlist(lapply(
    strsplit(as.character(unlist(best_models[61:90])), " ~"),
    '[',
    1
  )),
...
Biomass_refvar <- results %>%
  dplyr::filter(RESPONSE == "Biomass") %>%
  arrange(STATECD, refvar)
# make a table to chart Biomass model results
Biomass_refvar <- results %>%
  filter(RESPONSE == "Biomass") %>%
...
Volume_refvar <- results %>%
  dplyr::filter(RESPONSE == "Volume") %>%
  arrange(STATECD, refvar)
...
return_estimates <- function(x) {
  ...
  syn_pred <- unlist(lapply(1:nrow(regdata), function(x) {
    sum(regdata[x, all.vars(f)][, -1] * eblup_fit$fit$estcoef$beta)
  }))
  syn_pred2 <- unlist(lapply(1:nrow(regdata), function(x) {
    sum(regdata[x, all.vars(f)][, -1] * mse_fit$est$fit$estcoef$beta)
  }))
  ...
  if (res == "Biomass") {
    data.frame(
      ...
      src = src,
      STATECD = statecd,
      surveyunit = regdata$surveyunit,
      COUNTYCD = regdata$COUNTYCD,
      COUNTYNAME = regdata$COUNTYNAME,
...
estimates <- do.call(
  rbind,
  lapply(1:nrow(best_models), function(x) return_estimates(x))
)
fwrite(
  estimates,
  file.path(path_gedi, "Best_model_EBLUPs.csv"),
  replace = FALSE
)
best_summary <- estimates %>%
  group_by(STATECD, res, src) %>%
  summarize(mean_ser = mean(SER)) %>%
  as.data.frame()
fwrite(
  best_summary,
  file.path(path_gedi, "Best_model_SERs.csv"),
  replace = FALSE
)
...
coefficients <- do.call(
  rbind,
  lapply(1:nrow(best_models), function(x) return_coefs(x))
)
coef_wide <- coefficients %>%
  dplyr::select(-pvalue) %>%
  spread(predictor, value)
fwrite(
  coefficients,
  file.path(path_gedi, "Best_model_coeficients.csv"),
  replace = FALSE
)
fwrite(
  coef_wide,
  file.path(path_gedi, "Best_model_coeficients_wide.csv"),
  replace = FALSE
)
...
table(coefficients$predictor[coefficients$res == "Biomass"])[c(
  7,
  1:6,
  14,
  8:13
...
barplot_data <- data.frame(
  values = c(2, 5, 3, 3, 7, 8, 8, 6, 11, 12, 9, 9, 7, 7), # Create example data
  group = rep(c("B5", "B10", "B15", "B20", "B25", "B30", "B35"), each = 2),
  subgroup = c("Volume", "Biomass")
)
bp_data <- reshape(
  barplot_data, # Modify data for Base R barplot
  idvar = "subgroup",
  timevar = "group",
  direction = "wide"
)
...
png(
  file.path(path_gedi, "/results/Coeff_Freqs.png"),
  width = 5,
  height = 5,
  units = "in",
  res = 400
...
barplot(
  height = bp_data, # Grouped barplot using Base R
  beside = TRUE,
  horiz = TRUE,
  las = 1,
  axisnames = FALSE,
...
axis(
  side = 2,
  labels = colnames(bp_data),
  at = c(1.5, 4, 6.5, 9, 11.5, 14, 16.5),
  hadj = 0.625,
  lwd.ticks = 0
)
dev.off()
...
estimates %>%
  filter(STATECD == 37, src == "GEDI_default", surveyunit == 2) %>%
  dplyr::select(direct_est, FH_est) %>%
  plot()
abline(0, 1, col = "blue")

estimates %>%
  filter(STATECD == 37, src == "GEDI_default", surveyunit == 2, direct_est > 22)
...
return_best <- function(x) {
  ...
  if (!mse_fit$est$fit$convergence) {
    stop(
      "Model: ",
      paste(best_models[x, 3], "did not converge for statecd: "),
      statecd,
