...
countynames <- unlist(lapply(1:length(mapnames_default),function(x)substr(mapnames_default[x],start=5,stop=(nchar(mapnames_default[x])-4))))
...
system.time({               # runs in about 2 minutes for Virginia counties  #
  temp <- lapply(countynames,function(x){
    ...
    chm.exists = file.exists(file.path(path_chm_default,args,paste0("chm_",countyname,".tif")))
    cat("GEDI default CHM for county:",countyname,ifelse(chm.exists,"exists","does not exist"),'\n')
    if(chm.exists){
      test_tif <- raster(file.path(path_chm_default,args,paste0("chm_",countyname,".tif")))
...
write.csv(chm_dist,file.path(path_chm_default,args,"CHM_dist_by_county.csv"),row.names = F)

print(paste0("Default GEDI CHM bin distributions calculated for: ",statesUS$STATENAME[statesUS$STUSAB == args]))
...
countynames <- unlist(lapply(1:length(mapnames_NLCD),function(x)substr(mapnames_NLCD[x],start=5,stop=(nchar(mapnames_NLCD[x])-11))))
...
system.time({               # runs in about 2 minutes for Virginia counties  #
  temp <- lapply(countynames,function(x){
    ...
    statecd <- statesUS$STATECD[statesUS$STUSAB == args]
    countyname = test_poly$NAME # should be the same as x
    chm.exists = file.exists(file.path(path_chm_NLCD,args,paste0("chm_",countyname,"_forest.tif")))
    cat("GEDI NLCD CHM for county:",countyname,ifelse(chm.exists,"exists","does not exist"),'\n')
    if(chm.exists){
      test_tif <- raster(file.path(path_chm_NLCD,args,paste0("chm_",countyname,"_forest.tif")))
      # test_tif <- reproject_align_raster(rast = test_tif,ref_rast = lc30m)
      test_tif[test_tif > 100] <- NA  #remove water
      bins <- cut(test_tif[],breaks=c(0,5,10,15,20,25,30,35),labels=F)*5
      test_tif[] <- bins
...
print(paste0("NLCD GEDI CHM bin distributions calculated for: ",statesUS$STATENAME[statesUS$STUSAB == args]))
