...
path_chm_default <- file.path("/home/rstudio/data/GEDI/CHM/default",stateAbbrev)
...
if(!dir.exists(path_chm_default)) dir.create(path_chm_default)
if(!dir.exists(path_chm_NLCD)) dir.create(path_chm_NLCD)
...
system.time({     # takes about 3.5 minutes on charcoal2. Using beginCluster() doesn't speed it up.
  gediNAM30m_1_state <- raster::mask(raster::crop(gediNAM30m,counties_1_state),counties_1_state) # much faster to crop to the extent of the state polygon before masking
...
if(!file.exists(file.path(path_nlcd_1_state,paste0("NLCD_",stateAbbrev,"_aea.img")))){
  ...
  system.time({     # takes about 3-5 minutes on charcoal2
    NLCD_1_state <- raster::mask(raster::crop(NLCD,counties_1_state_aea),counties_1_state_aea) # much faster to crop to the extent of the state polygon before masking
  })
  ...
  if(args %in% c("North Carolina","Tennessee","Virginia")){
    ...
    raster::writeRaster(NLCD_1_state,filename=file.path(path_nlcd,
                                                        stateAbbrev,
                                                        paste0("NLCD_",
                                                               stateAbbrev,
                                                               "_aea.img")),overwrite=TRUE)
  }
}else{
  NLCD_1_state <- rast(file.path(path_nlcd_1_state,paste0("NLCD_",stateAbbrev,"_aea.img")))
}
...
test <- lapply(countiesFIA_1_state$COUNTYCD,function(countycode){
  county <- counties_1_state_aea[as.integer(counties_1_state_aea$CENSUSCODE) == countycode,]
  county_aea <- st_transform(county,raster::crs(NLCD_1_state))
  county <- st_transform(county,raster::crs(gediNAM30m_1_state))
  gedi_1_county <- raster::mask(raster::crop(gediNAM30m_1_state,county),county) # much faster to crop to the extent of the county polygon before masking
  system.time(gedi_1_county_aea <- reproject_align_raster(gedi_1_county,NLCD_1_state))
  # writeRaster(gedi_1_county,filename=file.path(path_chm_default,paste0("chm_",county$NAME,".tif")),
  #             overwrite=T,format="GTiff")
  writeRaster(gedi_1_county_aea,filename=file.path(path_chm_default,paste0("chm_",county$NAME,"_aea.tif")),
  ...
  # dev.off()
  png(filename=file.path(path_chm_default,paste0("chm_",county$NAME,"_aea.png")),
      width = 12,height = 12,units = "in",res = 300)
  plot(county_aea$geometry,main = county$NAME,border=1,lwd=1,col=NULL)
  plot(gedi_1_county_aea$Layer_1,add=TRUE)
  dev.off()
})
cat(paste(black$underline$bold(args),"Counties written to folder: ",path_chm_default))
...
reclass_df <- c(0, 40, 0,
                40, 49, 1,   # classifies 41, 42, and 43 as 1
                49, 51, 0,   
                51, 53, 1,   # classifies 52 as 1 (if this doesn't work change the second 52 to 53)
                53, 89, 0,
                89, 90, 1,   # classifies 90 as 1
                90, 95, 0)
...
remove_nonforest <- function(countycode,forest=NLCD_1_state){
  county <- counties_1_state_aea[as.integer(counties_1_state_aea$CENSUSCODE) == countycode,]
  fn_county <- file.path(path_chm_default,paste0("chm_",county$NAME[1],"_aea.tif"))
  if(!file.exists(fn_county)){
    stop("Error in function remove_nonforest: county file: ",blue$underline(fn_county)," does not exist")
  }
  ...
  if (ext(gedi_1_county_aea) == ext(forest)){
    ...
    forest_projected <- project(forest_cropped,gedi_1_county_aea,method='near')
  }
  ...
  # 
  writeRaster(test3,filename=file.path(path_chm_NLCD,paste0("chm_",county$NAME[1],"_forest.tif")),
              filetype="GTiff",overwrite=T)
  # saves a plot of the county chm
  png(filename=file.path(path_chm_NLCD,paste0("chm_",county$NAME[1],"_forest.png")),
